...
class _WatchShapeState extends State<WatchShape> {
  ...
  try {
    final dynamic result = await _channel.invokeMethod('getShape');
    if (result is int) {
      return result == 1 ? Shape.square : Shape.round;
    } else {
      // Manejar el caso en el que el resultado no sea un entero
      debugPrint('Error detecting shape: Result is not an integer');
      return Shape.round;
    }
  } on PlatformException catch (e) {
    // Manejar excepciones de plataforma
